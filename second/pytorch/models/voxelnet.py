...
from second.pytorch.models import middle, middle_student, middle_teacher, pointpillars, rpn, voxel_encoder
from torchplus import metrics
from second.pytorch.utils import torch_timer
from second.ops import Voxelization

#from second.core.iou3d.iou3d_utils import boxes_aligned_iou3d_gpu
...
class VoxelNet(nn.Module):
    def __init__(self,
                 output_shape, # output_shape = [1,z,y,x,16]
                 num_class=2, # the number of class
                 num_input_features=4, #
                 vfe_class_name="VoxelFeatureExtractor", # vfe_class_name = 'SimpleVoxel'
                 vfe_num_filters=[32, 128], # vfe_num_filters = [16]
                 with_distance=False, # with_distance = false
                 middle_class_name="SparseMiddleExtractor", # middle_class_name = 'SpMiddleFHD'
                 middle_num_input_features=-1, # middle_num_input_features = 4
                 middle_num_filters_d1=[64],
                 middle_num_filters_d2=[64, 64],
                 rpn_class_name="RPN", # rpn_class_name = 'RPNV2'
                 rpn_num_input_features=-1, # rpn_num_input_features
                 rpn_layer_nums=[3, 5, 5], #[5]
                 rpn_layer_strides=[2, 2, 2], #[1]
                 rpn_num_filters=[128, 128, 256], # [128]
                 rpn_upsample_strides=[1, 2, 4], # [0.5]
                 rpn_num_upsample_filters=[256, 256, 256], # [128]
                 use_norm=True,
                 use_groupnorm=False,
                 num_groups=32,
        ...
        # self.voxel_generator = Voxelization([0.1, 0.1, 0.1], [-70.4, -70.4, -5.3, 70.4, 70.4, -0.3], 100, [-1, -1])
        ...
        # 老版本
        # self.small_classes = ["pedestrian"]
        # self.twowheel_class = ["bicycle", "motorcycle"]
        # self.large_classes = ["car"]
        # self.fourwheel_class = ["truck", "bus"]

        # 新版本
        self.small_classes = ["pedestrian"]
        self.twowheel_class = ["bicycle", "tricycle"]
        self.large_classes = ["car"]
        self.fourwheel_class = ["truck", "bus", "semitrailer"]

        small_num_anchor_loc = sum([self.target_assigner.num_anchors_per_location_class(c) for c in self.small_classes])
        twowheel_num_anchor_loc = sum([self.target_assigner.num_anchors_per_location_class(c) for c in self.twowheel_class])
        large_num_anchor_loc = sum([self.target_assigner.num_anchors_per_location_class(c) for c in self.large_classes])
        fourwheel_num_anchor_loc = sum([self.target_assigner.num_anchors_per_location_class(c) for c in self.fourwheel_class])
        ...
        # knowledge_distillation = 1 : teacher
        # knowledge_distillation = 0 : student
        if knowledge_distillation:
            self.middle_feature_extractor = middle_teacher.get_middle_class(middle_class_name)(
                 output_shape,
                 use_norm,
            ...
            self.middle_feature_extractor = middle_student.get_middle_class(middle_class_name)(
                output_shape,
                use_norm,
                num_input_features=middle_num_input_features,
                num_filters_down1=middle_num_filters_d1,
                num_filters_down2=middle_num_filters_d2)
            rpn_num_input_features = 192
        ...
        self.twowheel_head = DefaultHead(
            num_filters=np.sum(self.rpn._num_upsample_filters),
            num_class=self._num_class,
            num_anchor_per_loc=twowheel_num_anchor_loc,
            encode_background_as_zeros=self._encode_background_as_zeros,
            use_direction_classifier=self._use_direction_classifier,
            box_code_size=self._box_coder.code_size,
            num_direction_bins=self._num_direction_bins,
        ...
        self.fourwheel_head = DefaultHead(
            num_filters=np.sum(self.rpn._num_upsample_filters),
            num_class=self._num_class,
            num_anchor_per_loc=fourwheel_num_anchor_loc,
            encode_background_as_zeros=self._encode_background_as_zeros,
            use_direction_classifier=self._use_direction_classifier,
            box_code_size=self._box_coder.code_size,
            num_direction_bins=self._num_direction_bins,
    ...
    def loss(self, example, preds_dict):
        # #####*****  box_preds shape is 4*7*87*87*7  *****#####
        # #####*****  cls_preds shape is 4*7*87*87*4  *****#####
        box_preds = preds_dict["box_preds"]
        cls_preds = preds_dict["cls_preds"]
        batch_size_dev = cls_preds.shape[0]
        self.start_timer("loss forward")
        # #####*****  labels shape is 4*52983  *****#####
        # #####*****  reg_targets shape is 4*52983*7  *****#####
        # #####*****  importance shape is 4*52983  *****#####
    ...
    def network_forward(self, voxels, num_points, coors, batch_size):
        ...
        # new voxel! add by chenxiangyang
        #voxel_features = self.voxel_feature_extractor(voxels, num_points,
        #                                              coors) 
        voxel_features, coors = self.voxel_feature_extractor(voxels, coors)
        # print(f'voxel_features shape is {voxel_features.shape}')
        # print(f'voxel FE shape is {voxel_features.shape}')
        ...
        t2 = time.time()
        #print("spconv cost : {}".format((t2 - t1) * 1000))
        # print(f'spatial shape is {spatial_features.shape}')
        ...
        twowheel = self.twowheel_head(rpn_out["out"])
        large = self.large_head(rpn_out["out"])
        fourwheel = self.fourwheel_head(rpn_out["out"])
        self.end_timer("rpn forward")
        # concated preds MUST match order in class_settings in config.
        res = {
            "box_preds": torch.cat([fourwheel["box_preds"], large["box_preds"], twowheel["box_preds"], small["box_preds"]], dim=1),
            "cls_preds": torch.cat([fourwheel["cls_preds"], large["cls_preds"], twowheel["cls_preds"], small["cls_preds"]], dim=1),
        }
        if self._use_direction_classifier:
            res["dir_cls_preds"] = torch.cat([fourwheel["dir_cls_preds"], large["dir_cls_preds"], twowheel["dir_cls_preds"], small["dir_cls_preds"]], dim=1)
        return res
        # print(preds_dict.keys())
        # self.end_timer("rpn forward")
        # return rpn_out

    def forward(self, example, knowledge_distillation=False):
        voxels = example["voxels"]
        # #####*****  num_points is 13344  *****#####
        num_points = None
        coors = example["coordinates"]
        """
        if len(num_points.shape) == 2:  # multi-gpu   don't run it
        ...
        # print(f'preds_dict dir_cls_pres is {preds_dict["dir_cls_preds"].shape}')
        # need to check size.
        ...
        if self.training:
            return self.loss(example, preds_dict), preds_dict
        elif knowledge_distillation:
            return preds_dict
        else:
    ...
    def predict(self, example, preds_dict):
        ...
        for box_preds, cls_preds, dir_preds, a_mask, meta in zip(
            ...
            if self._encode_background_as_zeros:
                ...
                # total_scores = F.softmax(cls_preds, dim=-1)
                # print(total_scores[1, :])
            else:
            ...
            if self._use_rotate_nms:
                nms_func = box_torch_ops.rotate_nms_mmcv        # 以后这个地方都是用mmcv的nms，不再使用原有的nms
            else:
            ...
            #print(f'before NMS is {selected_boxes.shape}')
            if self._multiclass_nms:
                boxes_all=selected_boxes
                selected_all = nms_func(
                    boxes_all,
                    selected_scores,
                    pre_max_size=500,
                    post_max_size=60,
                    iou_threshold=0.000000000000000001,
                )
                print(f'selected_all is {selected_all.shape}')
                selected_boxes = selected_boxes[selected_all]
                if self._use_direction_classifier:
                    selected_dir_labels = selected_dir_labels[selected_all]
                selected_labels = selected_labels[selected_all]
                selected_scores = selected_scores[selected_all]

            #print(f'after NMS is {selected_boxes.shape}')
            #############end##########################################
