...
def inference_video(args):
    ...
    for i in range(len(files)):
        ...
        print("voxel_generator time : {}".format(round((time.time() - t1) * 1000, 3)))
        voxels = res["voxels"]
        num_points = res["num_points_per_voxel"]
        coords = res["coordinates"]
        # np.save("/data/CLionProjects/voxelization/res/python/" + files[i].split("/")[-1].split(".")[0] + "_voxel.npy", voxels)
        # np.save("/data/CLionProjects/voxelization/res/python/" + files[i].split("/")[-1].split(".")[0] + "_num.npy", num_points)
        # np.save("/data/CLionProjects/voxelization/res/python/" + files[i].split("/")[-1].split(".")[0] + "_coors.npy", coords)
        ...
        t2 = time.time()
        print(
            files[i].split("/")[-1] + "; detect {} boxes; inference cost : {} ms!".format(pred["box3d_lidar"].shape[0],
                                                                                          round((t2 - t1) * 1000, 3)))
...
def main():
    ...
    parser.add_argument('--pointcloud',
                        default="/data/second_dv/datasets/gate8_double/test/bin",
                        help='pointcloud file or dir')
    parser.add_argument('--config', default="/data/second_dv/all.wjdata.semi.config", help='config file')
    parser.add_argument('--checkpoint', default="/data/second_dv/second/trainmodel/voxelnet-17000.tckpt",
                        help='Checkpoint file')
    parser.add_argument('--bmp', default=None, help='bmp_img')
    parser.add_argument('--score_thr', type=float, default=0.1, help='bbox score threshold')
    parser.add_argument('--save_dir', type=str, default=None,
                        help='dir to save results')
    parser.add_argument('--show', default=False, help='show online visualization results')
    parser.add_argument('--gt_label', default=None,
                        help='show one by one')
    parser.add_argument('--kd', default=False, help='Knowledge distillation')
