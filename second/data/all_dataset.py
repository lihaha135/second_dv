...
from second.data.wj_dataset import WjDataset
...
def create_groundtruth_database(dataset_class_name,
    ...
    for j in prog_bar(list(range(len(dataset)))):
        ...
        if gt_boxes.shape[0] != names.shape[0]:
            print("error")
        group_dict = {}
...
def create_groundtruth_database_parallel(dataset_class_name,
    ...
    root_path = Path(data_path)
    if database_save_path is None:
        database_save_path = root_path / 'gt_database1'
    else:
        database_save_path = Path(database_save_path)
    if db_info_save_path is None:
        db_info_save_path = root_path / "kitti_dbinfos_val1.pkl"
    database_save_path.mkdir(parents=True, exist_ok=True)
    all_db_infos = {}
