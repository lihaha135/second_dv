...
def clean_data(gt_anno, dt_anno, current_class, difficulty):
    #'car:5', 'pedestrian:5', 'bicycle:5', 'bus:5', 'truck:5','semitrailer:5','tricycle:5'
    #CLASS_NAMES = ['car', 'pedestrian', 'cyclist', 'van', 'person_sitting', 'truck']
    #CLASS_NAMES = ['car', 'pedestrian', 'bicycle', 'bus', 'truck', 'semitrailer','tricycle']
    #CLASS_NAMES = ['pedestrian', 'bicycle', 'tricycle', 'car', 'truck', 'bus']
    #CLASS_NAMES =['pedestrian','unkonw','bicycle','tricycle','car', 'van', 'truck','tool_cart','bus', 'semitrailer']
...
def calculate_iou_partly(gt_annos, dt_annos, metric, num_parts=50):
    ...
    for num_part in split_parts:
        ...
        if metric == 0:
            ...
            #rots = np.concatenate([a["rotation_y"] for a in gt_annos_part], 0)
            yaws = np.concatenate([a["alpha"] for a in gt_annos_part], 0)
            #gt_boxes = np.concatenate(
            #    [loc, dims, rots[..., np.newaxis]], axis=1)
            gt_boxes = np.concatenate(
                [loc, dims, yaws[..., np.newaxis]], axis=1)
            loc = np.concatenate([a["location"] for a in dt_annos_part], 0)
            dims = np.concatenate([a["dimensions"] for a in dt_annos_part], 0)
            #rots = np.concatenate([a["rotation_y"] for a in dt_annos_part], 0)
            yaws = np.concatenate([a["alpha"] for a in dt_annos_part], 0)
            #dt_boxes = np.concatenate(
            #    [loc, dims, rots[..., np.newaxis]], axis=1)
            dt_boxes = np.concatenate(
                [loc, dims, yaws[..., np.newaxis]], axis=1)
            ...
            #    np.float64)
            overlap_part =rotate_nms_cc_new(gt_boxes, dt_boxes)
...
def get_official_eval_result(gt_annos, dt_annos, current_classes, PR_detail_dict=None):
    ...
    for j, curcls in enumerate(current_classes):
        ...
        '''
            ...
            #result += print_str((f"3d   AP:{mAP3d_R40[j, 2, i]:.4f}"))
            #if compute_aos:
